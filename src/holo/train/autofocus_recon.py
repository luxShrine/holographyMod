...
def train_autofocus(
    ...
    # Swap to transforms.Grayscale(num_output_channels=3) before normalizing for grayscale imgs.
    ...
    # Prepare Dataset
    full_dataset = HQDLHMDataset(metadata_csv, crop=crop_size)
    print(full_dataset.bin_edges[:10])
    print(np.unique(full_dataset.bin_ids).size)
    ...
    class TransformedDataset(Dataset[tuple[Any, Any]]):
        ...
        def __getitem__(self, index: int):  # type: ignore
            ...
            """
            item = self.subset[index]  # could be (img, z, λ, px)# type: ignore
            x = item[0]  # unpack *only* the image# type: ignore
            if self.transform:
                x = self.transform(x)
            # use your precomputed bin_ids for the label
            return x, full_dataset.bin_ids[index]  # type: ignore
    ...
    # Build model
    model = eh.get_model(1, backbone).to(device)

    # Loss & optimizer
    # criterion = nn.CrossEntropyLoss()  # measures of error
    criterion = nn.SmoothL1Loss()  # measures of error
    ...
    progress = Progress(
        ...
        eh.MetricColumn("val_mae", fmt="{:.1f} µm", style="green"),
    ...
    epoch_task = progress.add_task(
        "Epoch", total=epochs, avg_loss=0, val_loss=0, val_mae=0, lr=optimizer.param_groups[0]["lr"]
    )
    ...
    with progress:  # allow for tracking of progress
        for epoch in range(1, epochs + 1):
            ...
            epoch_val_loss, epoch_mae = eh.validate_epoch(model, val_loader, criterion, device, 0)

            # go to next step
            scheduler.step(epoch_mae)  # Step scheduler based on validation metric # type:ignore
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    ...
                    Val Loss={epoch_val_loss:.4f} | Val Mae={epoch_mae:.4f}"
                )

    logger.info(f"Training complete. Best Val mae: {best_mae:.4f}")
    ...
    plot_data_obj = plotPred(
        val_z_pred_list,  # type: ignore
        train_z_pred_list,  # type: ignore
        val_z_true_list,  # type: ignore
        train_z_true_list,  # type: ignore
        train_err.tolist(),
        val_err.tolist(),
